...
st.write("Download your fully cleaned dataset as an Excel Table, CSV, or export project assets.")
...
def convert_df_to_styled_excel(df: pd.DataFrame) -> bytes:
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # إضافة تنسيق جدول إكسيل الأزرق
        tab = Table(displayName="CleanedDataTable", ref=tab_range)
        ...
        for col in worksheet.columns:
            ...
            for cell in col:
                ...
                if len(val_str) > max_len:
                    max_len = len(val_str)
            # تحديد عرض العمود مع هامش بسيط وتحديد حد أقصى للحماية (مثلاً 50)
...
for key in possible_keys:
    if key in st.session_state and st.session_state[key] is not None:
        if isinstance(st.session_state[key], pd.DataFrame):
            ...
            st.success(f"✅ Active dataset detected automatically (from session context)!")
            break
...
if export_df is not None:
    ...
    st.error("❌ No active dataset found in memory.")